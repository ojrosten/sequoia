...
namespace sequoia
{
  /** \brief RAII type holding a request to Windows for a finer timer resolution.

      Windows otherwise ends a sleep only on a tick of its default timer, about every 15.6 ms, so
      each sleep is rounded up to a whole number of ticks. A resolution which is not positive is
      not requested, and nothing is requested on other platforms.
   */
