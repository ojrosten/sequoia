...
export module sequoia.test_framework:BinaryRelationships;
...
export namespace sequoia::testing
{
  namespace abs_detail
  {
    ...
    inline constexpr bool has_abs{requires(const T& x) { { abs(x) } -> std::same_as<NormType>; }};
  }

  template<class T, class NormType>
  inline constexpr bool has_abs{abs_detail::has_abs<T, NormType>};
