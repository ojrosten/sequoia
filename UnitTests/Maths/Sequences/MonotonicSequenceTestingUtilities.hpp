...
namespace sequoia::unit_testing
{
  namespace impl
  {
    ...
    {
      if(check_equality(logger, sequence.size(), prediction.size(), impl::concat_messages(description, "Size incorrect")))
      {
        if(!prediction.empty())
        {
          check_equality(logger, sequence.back(), prediction.back(), impl::concat_messages(description, "Back element wrong"));
          check_equality(logger, sequence.front(), prediction.front(), impl::concat_messages(description, "Front element wrong"));
        }
        ...
        {          
          ...
          check_equality(logger, *i, *i_prediction, impl::concat_messages(description, "Dereferenced iterator wrong" + mess));
          check_equality(logger, *ci, *ci_prediction, impl::concat_messages(description, "Dereferenced citerator wrong" + mess));

          check_equality(logger, sequence[d], prediction[d], impl::concat_messages(description, "operator[] wrong" + mess));  

          const auto shift{static_cast<int64_t>(prediction.size()) - d - 1};
          check_equality(logger, *(ri + shift), *(ri_prediction + shift), impl::concat_messages(description, "Dereferenced riterator wrong" + mess));
          check_equality(logger, *(cri + shift), *(cri_prediction + shift), impl::concat_messages(description, "Dereferenced criterator wrong" + mess));
        }
