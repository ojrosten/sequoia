...
namespace sequoia
{
  namespace unit_testing
  {
    ...
    void test_edges::test_partial_edge_shared_weight()
    {
      ...
      check_regular_semantics(LINE("Standard Semantics"), edge1, edge);      
      ...
      check_regular_semantics(LINE("Standard semantics with shared weight"), edge2, edge);

      edge_t edge3(2, 8);
      check_equality(LINE(""), edge3, edge_t{2, 8});

      check_regular_semantics(LINE("Standard semantics with one having a shared weight"), edge2, edge);
    ...
    void test_edges::test_partial_edge_indep_weight()
    {
      ...
      check_regular_semantics(LINE("Standard semantics"), edge2, edge);
    }
    ...
    void test_edges::test_embedded_partial_edge_indep_weight()
    {
      ...
      check_regular_semantics(LINE("Standard semantics"), edge2, edge1); 
    }
    ...
    void test_edges::test_plain_edge()
    {
      ...
      e2.host_node(3);
      check_equality(LINE("Change host"), e2, edge_t{3, 1});      
    ...
    void test_edges::test_weighted_edge()
    {
...

      {
        ...
        check_regular_semantics(LINE(""), e2, e1);
      }

      {
        ...
        e1.host_node(2);
        check_equality(LINE("Change host, no induced change in target"), e1, edge_t{2, 0, vector<int>{3, 2}});
    ...
    void test_edges::test_plain_embedded_edge()
    {
      ...
      e.host_node(4);
      check_equality(LINE("Change host"), e, edge_t{4, 4, 1});
      ...
      e1.host_node(6);
      check_equality(LINE("Change host"), e1, edge_t{6, inversion_constant<true>{}, 9});

      e1.target_node(8);
      check_equality(LINE("Induced change to host"), e1, edge_t{8, inversion_constant<true>{}, 9});
    ...
    void test_edges::test_embedded_edge_indep_weight()
    {
      ...
      e2.host_node(8);
      check_equality(LINE("Change host"), e2, edge_t{8, inversion_constant<true>{}, 1, 1.1});

      e2.target_node(7);
      check_equality(LINE("Induced change host"), e2, edge_t{7, inversion_constant<true>{}, 1, 1.1});
    ...
    void test_edges::test_embedded_edge_shared_weight()
    {
      ...
      using edge_t = embedded_edge<double, shared, utilities::protective_wrapper<double>>;
        ...
        e.host_node(9);
        check_equality(LINE("Change host node"), e, edge_t{9, 11, 0, -1.2});
        ...
        e2.host_node(5);
        check_equality(LINE("Change host node, inducing change in target"), e2, edge_t{5, inversion_constant<true>{}, 4, 0.0});
        check_regular_semantics(LINE("Standard semantics"), e2, e);
        ...
        check_regular_semantics(LINE("Standard semantics"), e3, e);
        check_regular_semantics(LINE("Standard semantics"), e2, e3);
    }
  }
}
