...
namespace sequoia::testing
{
  ...
  void build_artefacts_free_test::test_tlogs()
  {
...

    {
      ...
      std::ranges::sort(read, {}, &compilation_record::object);
      check(equality, "Round trip: the source first, then what was read", read, records);
    }
...

    {
      ...
      fs::create_directories(dir);
      for(const auto name : {"Gadget.cpp", "Gadget.cxx", "Gadget.cpp.obj", "Gadget.cxx.obj"}) write_to_file(project / name, "", std::ios_base::out);
  ...
  void build_artefacts_free_test::test_build_tree()
  {
...

    {
      ...
      write_to_file(root / "build.ninja",
                    ...
                    std::ios_base::out);
      check(equality,
            "The log's record of an object build.ninja no longer names is not read; a source the log omits is supplied",
