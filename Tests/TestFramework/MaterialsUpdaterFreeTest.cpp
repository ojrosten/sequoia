...
namespace sequoia::testing
{
  ...
  void materials_updater_free_test::run_tests()
  {
    ...
    check_exception_thrown<std::runtime_error>(report("Empty 'to' path"),   [&]() { soft_update("", working); });
    check_exception_thrown<std::runtime_error>(report("Empty 'from' path"), [&]() { soft_update(auxiliary, ""); });

    soft_update(auxiliary, working);
    check(weak_equivalence, report("Soft update"), working, predictive);

    check(equality, report("Ensure that a target file equivalent to its replacement is not replaced"),
                   read_to_string(working_materials() /= "DirToBeKept/Comments.txt"),
                   read_to_string(predictive_materials() /= "DirToBeKept/Comments.txt"));

    check(report("Ensure fidelity of previous check"),
              read_to_string(working_materials() /= "DirToBeKept/Comments.txt")
