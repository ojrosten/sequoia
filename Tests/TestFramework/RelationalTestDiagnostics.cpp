...
namespace sequoia::testing
{
  ...
  void relational_false_positive_diagnostics::basic_tests()
  {
    ...
    check(std::ranges::less{},          "<",  5, 4);
    check(std::ranges::less_equal{},    "<=", 5, 4);
    check(std::ranges::greater{},       ">",  4, 5);
  ...
  {
    {
      ...
      check(within_tolerance{0.5}, "", v.cbegin(), v.cend(), p.cbegin(), p.cend());
      check(within_tolerance{0.5}, "",
...

    {
      ...
      check(std::ranges::less{}, "<",v.cbegin(), v.cend(), p.cbegin(), p.cend());
    }
  }
  ...
  void relational_false_positive_diagnostics::container_tests()
  {
    ...
    check(within_tolerance{0.5}, "", comp_t{{2.2, -1.0}, {5.1}}, comp_t{{2.1, -1.1}, {3.7}});
  }
  ...
  void relational_false_negative_diagnostics::basic_tests()
  {
    ...
    check(within_tolerance{0.5}, "", 4.5, 5.0);
    check(within_tolerance{0.5}, "", 5.5, 5.0);

    check(std::ranges::less{},          "<",  4, 5);
    check(std::ranges::less_equal{},    "<=", 4, 5);
    check(std::ranges::greater{},       ">",  5, 4);
  ...
  void relational_false_negative_diagnostics::container_tests()
  {
    ...
    using comp_t = std::vector<std::vector<double>>;
    check(within_tolerance{0.5}, "", comp_t{{2.2, -1.0}, {5.1}}, comp_t{{2.1, -1.1}, {4.7}});
  }
}
