...
namespace sequoia::testing
{
  ...
  void path_false_positive_free_diagnostics::test_paths()
  {
    ...
    check(equivalence,
          reporter{"Inequivalence of directory/file"},
          working_materials().append("Stuff/A"),
    ...
    check(equivalence,
          reporter{"Inequivalence of differently named directories with the same contents"},
          working_materials().append("Stuff/A"),
          working_materials().append("Stuff/C"));

    check(equivalence,
          reporter{"Inequivalence of directories with the same files but different contents"},
          working_materials().append("Stuff/A"),
          working_materials().append("MoreStuff/A"));

    check(equivalence,
          reporter{"Inequivalence of directories with some common files"},
    ...
    check(equivalence,
          reporter{"File inequivalence when default file checking is used"},
    ...
    check(equivalence,
          reporter{"Range inequivalence when default file checking us used"},
    ...
    check(weak_equivalence,
          reporter{"Weak inequivalence of directories with some common files"},
          working_materials().append("MoreStuff/B"),
          working_materials().append("Stuff/B"));

    check(weak_equivalence,
          reporter{"Directory weak inequivalence when default file checking is used"},
          working_materials().append("CustomComparison/A"),
          working_materials().append("CustomComparison/B"));

    check(weak_equivalence,
          reporter{"Weak inequivalence of range when default file checking is used"},
  ...
  void path_false_negative_free_diagnostics::test_paths()
  {
    ...
    check(equivalence,
          reporter{"Equivalence of a directory to itself"},
          working_materials().append("Stuff/A"),
          working_materials().append("Stuff/A"));

    check(equivalence,
          reporter{"Equivalence of a directory, with sub-directories to itself"},
          working_materials().append("Stuff"),
          working_materials().append("Stuff"));

    check(equivalence,
          reporter{"Equivalence of identical directories in different locations"},
          working_materials().append("Stuff/C"),
          working_materials().append("SameStuff/C"));

    check(bespoke_path_equivalence,
          reporter{"File equivalence when .ignore is ignored"},
          working_materials().append("CustomComparison/A/DifferingContent.ignore"),
          working_materials().append("CustomComparison/B/DifferingContent.ignore"));

    check(bespoke_path_equivalence,
          reporter{"Range equivalence when .ignore is ignored"},
          std::vector<fs::path>{working_materials().append("CustomComparison/A/DifferingContent.ignore")},
          std::vector<fs::path>{working_materials().append("CustomComparison/B/DifferingContent.ignore")});

    check(weak_equivalence,
         reporter{"Weak equivalence of directories in with the same contents but different names"},
         working_materials().append("Stuff"),
         working_materials().append("SameStuff"));

    check(bespoke_path_weak_equivalence,
          reporter{"Weak equivalence when .ignore is ignored"},
          working_materials().append("CustomComparison/A"),
          working_materials().append("CustomComparison/B"));

    check(bespoke_path_weak_equivalence,
          reporter{"Weak equivalence of range when .ignore is ignored"},
          std::vector<fs::path>{{working_materials().append("CustomComparison/A")}},
          std::vector<fs::path>{{working_materials().append("CustomComparison/B")}});
  }
}
