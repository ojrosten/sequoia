...
namespace sequoia::testing
{
  ...
  {
    {
      ...
      check_semantics(report("Broken check invariant"), beast{1}, beast{2}, std::weak_ordering::equivalent);
      check_semantics(report("Broken check invariant"), beast{1}, beast{2}, std::weak_ordering::greater);
    }

    {
      using beast = regular_broken_less<int>;

      check_semantics(report(""), beast{1}, beast{2}, std::weak_ordering::less);
    }
  ...
  void orderable_regular_false_negative_diagnostics::test_regular_semantics()
  {
    ...
    check_semantics(report(""), beast{1}, beast{2}, std::weak_ordering::less);
    check_semantics(report(""), beast{2}, beast{1}, std::weak_ordering::greater);
  }
}
