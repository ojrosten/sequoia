...
namespace sequoia::testing
{
  namespace impl
  {
    ...
    struct node_detailed_equality_checker
    {
      ...
      {
        check_equality(merge(description, "Sizes different"), logger, nodes.size(), prediction.size());

        check_range(merge(description, "const_node_iter"), logger, nodes.cbegin_node_weights(), nodes.cend_node_weights(), prediction.cbegin_node_weights(), prediction.cend_node_weights());

        check_range(merge(description, "const_reverse_node_iter"), logger, nodes.crbegin_node_weights(), nodes.crend_node_weights(), prediction.crbegin_node_weights(), prediction.crend_node_weights());
      }
    };
    ...
    struct node_equivalence_checker
    {
      ...
      static void check(std::string_view description, test_logger<Mode>& logger, const Nodes& nodes, equivalent_type prediction)
      {
        ...
        check_range(merge(description, "const_node_iter"), logger, nodes.cbegin_node_weights(), nodes.cend_node_weights(), prediction.begin(), prediction.end());
      }
    };
    ...
    struct node_equivalence_checker<Nodes, true>
    {
      ...
      {
        testing::check(merge(description, "Node storage should have zero size for empty node weights"), logger, nodes.empty());
      }
    };
  }
  ...
  struct detailed_equality_checker<maths::graph_impl::heterogeneous_node_storage<Ts...>>
  {
    ...
    {
      if(check_equality(merge(description, "Node storaage sizes different"), logger, nodes.size(), prediction.size()))
      {
  ...
  private:
    ...
    static void check_elements(std::string_view description, test_logger<Mode>& logger, const type& nodes, const type& prediction)
    {
      ...
      {
        const std::string message{merge(description, std::to_string(I) + "th element incorrect")};
        check_equality(merge(description, message), logger, nodes.template node_weight<I>(), prediction.template node_weight<I>());
  ...
  struct equivalence_checker<maths::graph_impl::heterogeneous_node_storage<Ts...>>
  {
    ...
    {
      if(check_equality(merge(description, "Node storage sizes different"), logger, nodes.size(), sizeof...(Ts)))
      {
        check_elements(description, logger, nodes, prediction);
      }
    }
  ...
  private:
    ...
    static void check_elements(std::string_view description, test_logger<Mode>& logger, const type& nodes, const equivalent_type& prediction)
    {
      ...
      {
        const std::string message{merge(description, std::to_string(I) + "th element incorrect")};
        check_equality(merge(description, message), logger, nodes.template node_weight<I>(), std::get<I>(prediction));
        check_elements<Mode, I+1>(description, logger, nodes, prediction);
      }
    }
  };
