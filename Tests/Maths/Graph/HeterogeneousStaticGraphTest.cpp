...
namespace sequoia::testing
{
  ...
  void test_heterogeneous_static_graph::test_generic_undirected()
  {
...

    {
      ...
      check_graph(report(""), g, {{edge{1, 0.5f}}, {edge{0, 0.5f}}}, std::tuple<int, double>{0, 0.0});
    }

    {
      constexpr auto g{make_undirected_graph()};
      using edge = typename decltype(g)::edge_init_type;
      check_graph(report(""), g, {{edge{1, -0.3f}}, {edge{0, -0.3f}}, {}}, std::tuple<float, int, double>{9.9f, -6, 11.2});
    }

    {
      ...
      check(equality, report(""), 4, g.get_node_weight<0>()(2));
    }
  }
