...
{
  [[nodiscard]]
  std::filesystem::path weighted_graph_test::source_file() const noexcept
  {
  ...
  void weighted_graph_test::execute_operations()
  {
    ...
    check(equality, report_line(""), graph.node_capacity(), 4_sz);
    graph.shrink_to_fit();
    check(equality, report_line("May fail if stl implementation doesn't actually shrink to fit!"), graph.node_capacity(), 0_sz);

    // NULL
    check_exception_thrown<std::out_of_range>(report_line("No nodes to erase"), [&graph](){ graph.erase_node(0); });
    check_exception_thrown<std::out_of_range>(report_line("No edges to erase"), [&graph](){ graph.erase_edge(graph.cbegin_edges(0)); });
    check_exception_thrown<std::out_of_range>(report_line("No nodes to set weight"), [&graph](){ graph.node_weight(graph.cend_node_weights(), 1.0); });
    check_exception_thrown<std::out_of_range>(report_line("No edges to set weight"), [&graph]() { graph.set_edge_weight(graph.cbegin_edges(0), EdgeWeight{1}); });

    check(equality, report_line(""), graph, graph_t{});

    check(equality, report_line("Node added with weight i"), graph.add_node(0.0, 1.0), 0_sz);
    //   (i)

    check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}}, {{0,1}}});
    check_semantics(report_line("Regular semantics"), graph, graph_t{});
    check_semantics(report_line("Regular semantics"), graph, graph_t{edge_init_list_t{{}}, {{1,0}}});

    check_exception_thrown<std::out_of_range>(report_line("Throw if node out of range"), [&graph](){ graph.node_weight(graph.cend_node_weights(), 0.0); });
    ...
    check(equality, report_line("Node of weight i recreated"), graph.add_node(0.0, 1.0), 0_sz);
    //    (i)
    ...
    check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}}, {{1.1,-4.3}}});
    ...
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{{edge_init_t{0,1}}}}, {{1.1,-4.3}}});
      check_semantics(report_line("Regular semantics"), graph, graph_t{edge_init_list_t{{{edge_init_t{0,2}}}}, {{1.1,-4.3}}});
      check_semantics(report_line("Regular semantics"), graph, graph_t{edge_init_list_t{{{edge_init_t{0,1}}}}, {{-4.3, 1.1}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1}, edge_init_t{0,1}}}, {{1.1,-4.3}}});
      check_semantics(report_line("Regular semantics"), graph, graph_t{{{edge_init_t{0,2}, edge_init_t{0,2}}}, {{1.1,-4.3}}});
      check_semantics(report_line("Regular semantics"), graph, graph_t{{{edge_init_t{0,1}, edge_init_t{0,1}}}, {{-4.3,1.1}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::directed_embedded)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,0,1,1}, edge_init_t{0,0,0,1}}}, {{1.1,-4.3}}});
      check_semantics(report_line(""), graph, graph_t{{{edge_init_t{0,0,1,2}, edge_init_t{0,0,0,2}}}, {{1.1,-4.3}}});
      check_semantics(report_line(""), graph, graph_t{{{edge_init_t{0,0,1,1}, edge_init_t{0,0,0,1}}}, {{-4.3,1.1}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1,1}, edge_init_t{0,0,1}}}, {{1.1,-4.3}}});
      check_semantics(report_line(""), graph, graph_t{{{edge_init_t{0,1,2}, edge_init_t{0,0,2}}}, {{1.1,-4.3}}});
      check_semantics(report_line(""), graph, graph_t{{{edge_init_t{0,1,1}, edge_init_t{0,0,1}}}, {{-4.3,1.1}}});
    }
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{{edge_init_t{0,-2}}}}, {{1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,-2}, edge_init_t{0,-2}}}, {{1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::directed_embedded)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,0,1,-2}, edge_init_t{0,0,0,-2}}}, {{1.1,-4.3}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,1,-2}, edge_init_t{0,0,-2}}}, {{1.1,-4.3}}});
    }
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{{edge_init_t{0,-4}}}}, {{1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,-4}, edge_init_t{0,-4}}}, {{1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::directed_embedded)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,0,1,-4}, edge_init_t{0,0,0,-4}}}, {{1.1,-4.3}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,1,-4}, edge_init_t{0,0,-4}}}, {{1.1,-4.3}}});
    }
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{{edge_init_t{0,-8}}}}, {{1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,-8}, edge_init_t{0,-8}}}, {{1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::directed_embedded)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,0,1,-8}, edge_init_t{0,0,0,-8}}}, {{1.1,-4.3}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,1,-8}, edge_init_t{0,0,-8}}}, {{1.1,-4.3}}});
    }
    ...
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,0,1,1}, edge_init_t{0,0,0,1}, edge_init_t{0,0,3,-1}, edge_init_t{0,0,2,-1}}}, {{1.1,-4.3}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1,1}, edge_init_t{0,0,1}, edge_init_t{0,3,-1}, edge_init_t{0,2,-1}}}, {{1.1,-4.3}}});
    }
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1}, edge_init_t{0,-4}}}, {{1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
    ...
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,0,1,1}, edge_init_t{0,0,0,1}, edge_init_t{0,0,3,-4}, edge_init_t{0,0,2,-4}}}, {{1.1,-4.3}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1,1}, edge_init_t{0,0,1}, edge_init_t{0,3,-4}, edge_init_t{0,2,-4}}}, {{1.1,-4.3}}});
    }


    check(equality, report_line(""), graph.add_node(), 1_sz);
    //   /\ 1
    //   \/
    // (1.1-i4.3)   (0)
    //   /\
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1}, edge_init_t{0,-4}, edge_init_t{1, 6}}, {}}, {{1.1,-4.3}, {}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
    ...
    {
      check(equality, report_line(""), graph,
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{{{}, {edge_init_t{1,1}, edge_init_t{1,-4}, edge_init_t{0, 6}}}, {{}, {1.1,-4.3}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1}, edge_init_t{0,-4}, edge_init_t{1, 7}}, {}}, {{1.1,-4.3}, {}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1}, edge_init_t{0,-4}, edge_init_t{1, 6}}, {edge_init_t{0, 7}}}, {{1.1,-4.3}, {}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
    ...
    check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}}, {{}}});
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{edge_init_t{0,1}}}, {{}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1}, edge_init_t{0,1}}}, {{}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::directed_embedded)
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,0,1,1}, edge_init_t{0,0,0,1}}}, {{}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{{{edge_init_t{0,1,1}, edge_init_t{0,0,1}}}, {{}}});
    }

    check(equality, report_line("Node with weight 1+i inserted into slot 0"), graph.insert_node(0, 1.0, 1.0), 0_sz);
    //       /\ 1
    //       \/
    // (1+i) (0)
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1}}}, {{1, 1}, {}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1}, edge_init_t{1,1}}}, {{1, 1}, {}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::directed_embedded)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1,1,1}, edge_init_t{1,1,0,1}}}, {{1, 1}, {}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1,1}, edge_init_t{1,0,1}}}, {{1, 1}, {}}});
    }
    ...
    if constexpr (GraphFlavour == graph_flavour::directed)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1}}}, {{2, 2}, {}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::undirected)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1}, edge_init_t{1,1}}}, {{2, 2}, {}}});
    }
    else if constexpr(GraphFlavour == graph_flavour::directed_embedded)
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1,1,1}, edge_init_t{1,1,0,1}}}, {{2, 2}, {}}});
    }
    else
    {
      check(equality, report_line(""), graph, graph_t{edge_init_list_t{{}, {edge_init_t{1,1,1}, edge_init_t{1,0,1}}}, {{2, 2}, {}}});
    }
  }
}
