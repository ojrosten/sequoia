...
namespace sequoia::testing
{
  ...
  void test_graph_update::check_setup(const Graph& graph)
  {
    ...
    if constexpr(GraphFlavour == flavour::undirected)
    {
      ...
      check(equality, report(""), graph, expected);
    }
  ...
  void test_graph_update::check_df_update(Graph graph)
  {
    ...
    check(equality, report(""), graph.cbegin_node_weights(), graph.cend_node_weights(), expectedNodeWeights.cbegin(), expectedNodeWeights.cend());
