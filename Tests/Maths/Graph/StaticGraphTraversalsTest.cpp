...
{
  [[nodiscard]]
  std::string_view test_static_graph_traversals::source_file() const noexcept
  {
    return __FILE__;
  }
  ...
  constexpr auto test_static_graph_traversals::arrage()
  {
    ...
    pseudo_depth_first_search(g, find_disconnected::yes, 0, null_func_obj{}, lateNodeFn);
  ...
  constexpr auto test_static_graph_traversals::bfs()
  {
    ...
    breadth_first_search(g, find_disconnected::yes, 0, null_func_obj{}, null_func_obj{}, edgeFn);
  ...
  {
    {
      ...
      check_equality(LINE(""), ordering[0], 0_sz);
      check_equality(LINE(""), ordering[1], 1_sz);
    }

    {
      constexpr auto data{bfs()};
      check_equality(LINE(""), data[0], 3_sz);
      check_equality(LINE(""), data[1], 0_sz);
    }
