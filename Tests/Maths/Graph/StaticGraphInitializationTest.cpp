...
{
  [[nodiscard]]
  std::string_view test_static_graph::source_file() const noexcept
  {
    return __FILE__;
  }
  ...
  void test_static_graph::test_generic_undirected()
  {
...

    {
      ...
      {
        check(equality, LINE("2 bytes for each half edge and 2 for the partition data"), sizeof(g_type), 4_sz);
      }
