...
namespace sequoia::testing
{
  ...
  void saturating_mul_test_base::execute_tests()
  {
    ...
    check_static<(saturating_mul(gubT, U{}) == value_t{})>();
    check_static<(saturating_mul(T{}, gubU) == value_t{})>();
    check_static<(saturating_mul(gubT, gubU) == gub)>();
    check_static<(saturating_mul(gubT, llbU) == (llbU == 0 ? value_t{} : llb))>();
    check_static<(saturating_mul(llbT, gubU) == (llbT == 0 ? value_t{} : llb))>();
    check_static<(saturating_mul(llbT, llbU) == (((llbT == 0) || (llbU == 0))? value_t{} : gub))>();
...
#pragma warning(pop)
  ...
  void saturating_add_test_base::execute_tests()
  {
    ...
    check_static<(saturating_add(gubT, gubU) == gub)>();
    check_static<(saturating_add(llbT, llbU) == llb)>();
