  ...
  {
    {
      ...
      check_exception_thrown<std::logic_error>(report("Empty string"), [](){ factory<int, double> f{{"int", ""}}; });
      check_exception_thrown<std::logic_error>(report("Empty string"), [](){ factory<int, double> f{{"", "bar"}}; });
      check_exception_thrown<std::logic_error>(report("Duplicated names"), [](){ factory<int, double> f{{"bar", "bar"}}; });

      factory<int, double> f{{"int", "double"}}, g{{"bar", std::string{"foo"}}};

      check(equivalence, report(""), f, prediction_type{{{"int", 0}, {"double", 0.0}}});
      check(equivalence, report(""), g, prediction_type{{{"bar", 0}, {"foo", 0.0}}});

      check_semantics(report(""), f, g);

      check_exception_thrown<std::runtime_error>(report(""), [&f](){ return f.make("plurgh"); });

      const auto created{f.make_or<int>("plurgh")};
      check(equality, report(""), created, std::variant<int, double>{0});
    }
...

    {
      ...
      check(equivalence, report(""), f, prediction_type{{{"x", regular_type{1}}, {"y", move_only_type{1}}}}, 1);
      check(equivalence, report(""), g, prediction_type{{{"make_x", regular_type{2}}, {"make_y", move_only_type{2}}}}, 2);
...

    {
      ...
      check(equivalence, report(""), f, prediction_type{ {{"foo-int", foo<int>{}}, {"foo-double", foo<double>{}}} });
      check(equivalence, report(""), g, prediction_type{ {{"int", foo<int>{42}}, {"double", foo<double>{42}}} }, 42);
