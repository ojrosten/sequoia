  ...
  namespace testing
  {
    ...
    void partitioned_data_test::test_static_storage()
    {
...

      {
        ...
        storage_t storage{{5}};
        check(equivalence, LINE(""), storage, prediction_t{{5}});

        storage_t storage2{{3}};
        check(equivalence, LINE(""), storage2, prediction_t{{3}});

        check_semantics(LINE(""), storage, storage2);

        auto iter = storage.begin_partition(0);
        *iter = 4;
        check(equality, LINE(""), storage, storage_t{{4}});
      }

      {
        ...
        check(equivalence, LINE(""), storage, prediction_t{{1.2, 1.1}, {2.6, -7.8}, {0.0, -9.3}});

        storage_t storage2{{1.2}, {1.1, 2.6, -7.8}, {0.0, -9.3}};
        check(equivalence, LINE(""), storage2, prediction_t{{1.2}, {1.1, 2.6, -7.8}, {0.0, -9.3}});
...

      {
        ...
        check(equivalence, LINE(""), storage, prediction_t{{0.2,0.3,-0.4}, {0.8,-1.1,-1.4}});
        check(equivalence, LINE(""), storage2, prediction_t{{0.2,0.3}, {-0.4, 0.8,-1.1,-1.4}});
...

      {
...

        {
          ...
          check(equivalence, LINE(""), storage, prediction_t{{ndc{1}}});
        }

        {
          ...
          check(equivalence, LINE(""), storage2, prediction_t{{ndc{1}, ndc{1}}, {ndc{0}}, {ndc{2}, ndc{4}}});
        }
      }
    }
    ...
    Storage partitioned_data_test::test_generic_storage()
    {
      ...
      // Null
      check(equivalence, LINE(""), storage, equivalent_type{});
      ...
      check(equality, LINE(""), storage, Storage{{}});
      ...
      check(equivalence, LINE(""), storage, equivalent_type{{2}, {}});
      check(equality, LINE(""), storage, Storage{{2}, {}});
      ...
      auto iter = storage.begin_partition(0);
      check(equality, LINE(""), *iter, 2);
      *iter = 3;
      // [3][]

      check(equality, LINE(""), storage, Storage{{3}, {}});

      storage.push_back_to_partition(1, 4);
      // [3][4]

      check(equivalence, LINE(""), storage, equivalent_type{{3}, {4}});
      check(equality, LINE(""), storage, Storage{{3}, {4}});
      check_semantics(LINE("Regular semantics"), storage, Storage{{4},{3}});

      storage.swap_partitions(1,0);
      // [4][3]

      check(equivalence, LINE(""), storage, equivalent_type{{4}, {3}});
      check(equality, LINE(""), storage, Storage{{4}, {3}});
      ...
      check(equivalence, LINE(""), storage, equivalent_type{{3}, {4}, {9, -3}});
      check(equality, LINE(""), storage, Storage{{3}, {4}, {9, -3}});
      check_semantics(LINE("Regular semantics"), storage, Storage{{4},{3}});

      storage.swap_partitions(1,2);
      // [3][9,-3][4]

      check(equality, LINE(""), storage, Storage{{3}, {9, -3}, {4}});
      ...
      check(equality, LINE(""), storage, Storage{{3}, {4}, {2, 9, -3}});
      check(equality, LINE(""), *iter, 2);
      check(equality, LINE(""), iter.partition_index(), 2_sz);

      storage.swap_partitions(0,2);
      // [2, 9,-3][4][3]

      check(equality, LINE(""), storage, Storage{{2, 9, -3}, {4}, {3}});
      ...
      check(equality, LINE(""), storage, Storage{{3}, {4}, {2, 8, 9, -3}});
      check(equality, LINE(""), *iter, 8);
      check(equality, LINE(""), iter.partition_index(), 2_sz);

      iter = storage.insert_to_partition(storage.begin_partition(2) + 4, 7);
      // [3][4][2, 8, 9,-3, 7]

      check(equality, LINE(""), storage, Storage{{3}, {4}, {2, 8, 9, -3, 7}});
      check(equality, LINE(""), *iter, 7);
      check(equality, LINE(""), iter.partition_index(), 2_sz);

      storage.insert_to_partition(storage.cbegin_partition(2) + 5, 5);
      // [3][4][2, 8, 9,-3, 7, 5]

      check(equality, LINE(""), storage, Storage{{3}, {4}, {2, 8, 9, -3, 7, 5}});

      storage.add_slot();
      storage.insert_to_partition(storage.cbegin_partition(2), 1);
      // [3][4][1, 2, 8, 9,-3, 7, 5][]

      check(equality, LINE(""), storage, Storage{{3}, {4}, {1, 2, 8, 9, -3, 7, 5}, {}});

      storage.insert_to_partition(storage.cbegin_partition(0), 12);
      // [12, 3][4][1, 2, 8, 9,-3, 7, 5][]

      check(equality, LINE(""), storage, Storage{{12, 3}, {4}, {1, 2, 8, 9, -3, 7, 5}, {}});

      storage.swap_partitions(2, 1);
      // [12, 3][1, 2, 8, 9,-3, 7, 5][4][]

      check(equality, LINE(""), storage, Storage{{12, 3}, {1, 2, 8, 9, -3, 7, 5}, {4}, {}});

      storage.swap_partitions(0,1);
      // [1, 2, 8, 9,-3, 7, 5][12, 3][4][]

      check(equality, LINE(""), storage, Storage{{1, 2, 8, 9, -3, 7, 5}, {12, 3}, {4}, {}});
      ...
      check(equality, LINE(""), storage, Storage{{12, 13, 3}, {4}, {1, 2, 8, 9, -3, 7, 5}, {}});

      storage.insert_to_partition(storage.cbegin_partition(0) + 3, -8);
      // [12, 13, 3, -8][4][1, 2, 8, 9,-3, 7, 5][]

      check(equality, LINE(""), storage, Storage{{12, 13, 3, -8}, {4}, {1, 2, 8, 9, -3, 7, 5}, {}});

      iter = storage.erase_from_partition(0, 0);
      // [13, 3, -8][4][1, 2, 8, 9,-3, 7, 5][]

      check(equality, LINE(""), storage, Storage{{13, 3, -8}, {4}, {1, 2, 8, 9, -3, 7, 5}, {}});
      check(equality, LINE(""), *iter, 13);
      check(equality, LINE(""), iter.partition_index(), 0_sz);

      storage.erase_from_partition(0, 2);
      // [13, 3][4][1, 2, 8, 9,-3, 7, 5][]

      check(equality, LINE(""), storage, Storage{{13, 3}, {4}, {1, 2, 8, 9, -3, 7, 5}, {}});

      iter = storage.erase_from_partition(storage.cbegin_partition(0));
      // [3][4][1, 2, 8, 9,-3, 7, 5][]

      check(equality, LINE(""), storage, Storage{{3}, {4}, {1, 2, 8, 9, -3, 7, 5}, {}});
      check(equality, LINE(""), *iter, 3);
      check(equality, LINE(""), iter.partition_index(), 0_sz);
      ...
      check(equality, LINE(""), storage, Storage{{3}, {4}, {-2, 1, 2, 8, 9, -3, 7, 5}, {-2}});
      ...
      check(equality, LINE(""), storage, Storage{{3}, {4}, {-2, 1, 2, 8, 9, -3, 7, 5}, {-4, -2, -4}});
      ...
      if(check(equality, LINE(""), storage, Storage{{1}, {3}, {1}, {4}}))
      {
        ...
        *iter = 1;
        // [1][3][1][4]
        check(equality, LINE(""), storage, Storage{{1}, {3}, {1}, {4}});
      ...
      check(equality, LINE(""), storage, Storage{{2, 1, 2}, {3}, {1}, {4}});

      storage.insert_to_partition(storage.cbegin_partition(2), 7);
      // [2,1,2][3][1, 7][4]

      check(equality, LINE(""), storage, Storage{{2, 1, 2}, {3}, {7,1}, {4}});

      storage.erase_slot(2);
      // [2,1,2][3][4]

      check(equality, LINE(""), storage, Storage{{2,1,2}, {3}, {4}});
      ...
      if(check(equality, LINE(""), storage, Storage{{3, -5}, {4, -5}}))
      {
        iter = storage.begin_partition(0);
        ++iter;
        *iter = 2;
        ...
        *iter = -5;
        // [3,-5][4,-5]
        check(equality, LINE(""), storage, Storage{{3, -5}, {4, -5}});
      ...
      check(equality, LINE(""), storage, Storage{{3, -5}, {4, -5}, {8, 9}});

      storage.erase_slot(1);
      // [3,-5][8,9]

      check(equality, LINE(""), storage, Storage{{3, -5}, {8, 9}});

      storage.erase_slot(0);
      // [8,9]

      check(equality, LINE(""), storage, Storage{{8, 9}});
      ...
      check(equality, LINE(""), storage, Storage{{8, 9}, {4, -5}});

      {
        ...
        *iter = -5;
        ...
        check(equality, LINE(""), *next, -5);
      }
      // [-5][4,-5]

      check(equality, LINE(""), storage, Storage{{-5}, {4, -5}});
      ...
      check(equality, LINE(""), storage, Storage{{-5}, {4}});

      storage.push_back_to_partition(0, 6);
      storage.push_back_to_partition(1, --storage.cend_partition(0));
      // [-5,6][4,6]

      check(equality, LINE(""), storage, Storage{{-5, 6}, {4, 6}});

      storage.push_back_to_partition(0, -2);
      storage.push_back_to_partition(1, --storage.cend_partition(0));
      // [-5,6,-2][4,6,-2]

      check(equality, LINE(""), storage, Storage{{-5, 6, -2}, {4, 6, -2}});
      ...
      if(check(LINE(""), found != storage.end_partition(0)))
        check(equality, LINE(""), *found, 6);

      erase_from_partition_if(storage, 0, [](const int& elt) { return elt == 6; });
      // [-5,-2][4,6,-2]

      check(equality, LINE(""), storage, Storage{{-5, -2}, {4, 6, -2}});
      ...
      check(equality, LINE(""), storage.size(), 5_sz);
      ...
      check(equality, LINE(""), storage, Storage{{-5, -2}, {4, 6, -2, 7, 7}});
    ...
    void partitioned_data_test::test_generic_iterator_properties()
    {
      ...
      {
        check(equality, LINE(""), iter.operator->(), &Handler<int>::get(*vec.begin()));
      }
      else
      {
        check(equality, LINE(""), iter.operator->(), &Handler<int>::get(*vec.cbegin()));
      }


      check(equality, LINE(""), Handler<int>::get(vec[0]),*iter);
      check(equality, LINE(""), iter.partition_index(), 4_sz);

      ++iter;
      check(equality, LINE(""), *iter, Handler<int>::get(vec[1]));
      check(equality, LINE(""), iter.partition_index(), 4_sz);

      iter++;
      check(equality, LINE(""), *iter, Handler<int>::get(vec[2]));
      ...
      iter--;
      check(equality, LINE(""), *iter, Handler<int>::get(vec[0]));

      check(equality, LINE(""), iter[0], Handler<int>::get(vec[0]));
      check(equality, LINE(""), iter[1], Handler<int>::get(vec[1]));
      check(equality, LINE(""), iter[2], Handler<int>::get(vec[2]));

      iter+=1;
      check(equality, LINE(""), iter[0], Handler<int>::get(vec[1]));
      ...
      auto iter2 = iter + 2;
      check(equality, LINE(""), iter[0], Handler<int>::get(vec[0]));
      check(equality, LINE(""), iter2[0], Handler<int>::get(vec[2]));

      auto iter3 = iter2 - 2;
      check(equality, LINE(""), iter3[0], Handler<int>::get(vec[0]));
      ...
      check(equality, LINE(""), std::ptrdiff_t{-2}, distance(iter2, iter3));
      check(equality, LINE(""), std::ptrdiff_t{2}, distance(iter, iter2));

      auto std_iter = iter.base_iterator();

      check(equality, LINE(""), Handler<int>::get(*std_iter), 1);
    }
    ...
    void partitioned_data_test::test_contiguous_capacity()
    {
      ...
      s.reserve(4);
      check(equality, LINE(""), s.capacity(), 4_sz);
      check(equality, LINE(""), s.num_partitions_capacity(), 0_sz);

      s.reserve_partitions(8);
      check(equality, LINE(""), s.capacity(), 4_sz);
      check(equality, LINE(""), s.num_partitions_capacity(), 8_sz);
    ...
    void partitioned_data_test::test_bucketed_capacity()
    {
      ...
      check(equality, LINE(""), s.num_partitions_capacity(), 4_sz);
      if constexpr(ThrowOnRangeError) check_exception_thrown<std::out_of_range>(LINE(""), [&s](){ return s.partition_capacity(0); });

      s.shrink_num_partitions_to_fit();
      check(equality, LINE("May fail if shrink to fit impl does not reduce capacity"), s.num_partitions_capacity(), 0_sz);
      if constexpr(ThrowOnRangeError) check_exception_thrown<std::out_of_range>(LINE(""), [&s](){ return s.partition_capacity(0); });

      s.add_slot();
      check(equality, LINE(""), s.partition_capacity(0), 0_sz);
      ...
      check(equality, LINE(""), s.partition_capacity(0), 4_sz);

      s.shrink_to_fit(0);
      check(equality, LINE("May fail if shrink to fit impl does not reduce capacity"), s.partition_capacity(0), 0_sz);
