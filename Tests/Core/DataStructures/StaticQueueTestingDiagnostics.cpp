...
namespace sequoia::testing
{
  ...
  void test_static_queue_false_positives::check_depth_0()
  {
    ...
    check(report("Empty queue must be empty"), !s.empty());
    check(equality, report("Empty queue must have size zero"), s.size(), 1_sz);
  }
  ...
  void test_static_queue_false_positives::check_depth_1()
  {
    ...
    check(equality, report("Empty queue versus populated queue"), s, t);

    s.push(2);
    check(equality, report("Differing elements"), s, t);
