...
{  
  ...
  struct detailed_equality_checker<data_structures::static_priority_queue<T, MaxDepth, Compare>>
  {
    ...
    static void check(std::string_view description, test_logger<Mode>& logger, const type& queue, const type& prediction)
    {
      ...
      {
        check_equality(merge(description, "Inconsistent top element"), logger, queue.top(), prediction.top());
      }

      check_equality(merge(description, "Inconsistent Hidden state"), logger, prediction == queue, true);
      check_equality(merge(description, "Inconsistent Hidden state, symmetry of =="), logger, queue == prediction, true);
    }
  };
}
