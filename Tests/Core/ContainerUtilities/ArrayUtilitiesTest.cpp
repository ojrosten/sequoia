...
namespace sequoia::testing
{
  ...
  void array_utilities_test::run_tests()
  {
    ...
    {
      check_exception_thrown<std::logic_error>(report(""), [](){ return to_array<int,0>({1});});

      constexpr auto a{to_array<int, 0>({})};
      check(equality, report(""), a, {});

    }

    {
      check_exception_thrown<std::logic_error>(report(""), [](){ return to_array<int,5>({0,1,2,3,4,5});});

      constexpr auto a{to_array<int, 5>({3, 6, 9, 1, 0})};
      check(equality, report(""), a, {3, 6, 9, 1, 0});
    }

    {
      check_exception_thrown<std::logic_error>(report(""), [](){ return to_array<int,3>({0,1,2,3});});

      constexpr auto a{to_array<int, 3>({1, 2, 3}, [](const int e) { return e * 3; })};
      check(equality, report(""), a, {3, 6, 9});
    }

    {
      ...
      check_exception_thrown<std::logic_error>(report(""), [converter](){ return to_array<ndc_t,2>({0,1,2}, converter);});

      constexpr std::array<no_default_constructor, 2>
        a{to_array<ndc_t, 2>({2, 3}, converter)};

      check(equality, report(""), a, {ndc_t{2}, ndc_t{3}});
    }
  }
}
